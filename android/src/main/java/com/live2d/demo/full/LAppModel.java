...
//         前回セーブされた状態をロード
    ...
    public void draw(CubismMatrix44 matrix) {
        if (model == null) {
            if (LAppDefine.DEBUG_LOG_ENABLE) {
                android.util.Log.d("LAppModel", "draw: model is null, skipping draw");
            }
            return;
        }
    ...
    private void setupModel(ICubismModelSetting setting) {
        ...
        // Load Cubism Model
        {
            ...
            if (!fileName.equals("")) {
                ...
                LAppPal.printLog("setupModel: Buffer size: " + (buffer != null ? buffer.length : "null"));
                
                if (buffer != null) {
                    loadModel(buffer, mocConsistency);
                    LAppPal.printLog("setupModel: Model loaded, getModel() result: " + (getModel() != null ? "not null" : "null"));
                } else {
        ...
        // Physics
        {
            ...
            if (!path.equals("")) {
                ...
                byte[] buffer = createBufferFromFileSystem(modelPath);
        ...
        if (modelSetting.getLayoutMap(layout)) {
            ...
            if (LAppDefine.DEBUG_LOG_ENABLE) {
                ...
                for (Map.Entry<String, Float> entry : layout.entrySet()) {
                    android.util.Log.d("LAppModel", "Layout: " + entry.getKey() + " = " + entry.getValue());
                }
            }
        } else {
            if (LAppDefine.DEBUG_LOG_ENABLE) {
    ...
    private void setupModelFromFileSystem(ICubismModelSetting setting, String modelDirectory) {
        modelSetting = setting;
     ...
     */
    private void setupTexturesFromFileSystem() {
        ...
        LAppPal.printLog("setupTexturesFromFileSystem: Texture count: " + modelSetting.getTextureCount());
     ...
     * フレームバッファ以外の描画先
     */
    private void preLoadMotionGroupFromFileSystem(final String group) {
        final int count = modelSetting.getMotionCount(group);
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (!path.equals("")) {
                ...
                byte[] buffer;
                buffer = createBufferFromFileSystem(modelPath);
